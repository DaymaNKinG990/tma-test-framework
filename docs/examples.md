...
async def basic_bot_example():
    ...
    async with TelegramBot(config.bot_token, config) as bot:
        # Get bot information
...
async def get_mini_app_example():
    ...
    async with TelegramBot(config.bot_token, config) as bot:
        # Get Mini App URL from bot
        mini_app_ui = await bot.get_mini_app(
            user_id=123456789,
            start_param="test_param"
        )
        
        print(f"Mini App URL: {mini_app_ui.url}")
        print(f"Start Param: {mini_app_ui.start_param}")
