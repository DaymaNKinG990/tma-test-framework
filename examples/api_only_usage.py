...
async def test_mini_app_api_only():
    ...
    try:
        ...
        print(f"   GET /api/status: {'[OK]' if get_result.success else '[ERROR]'} "
        ...
        print(f"   POST /api/data: {'[OK]' if post_result.success else '[ERROR]'} "
        ...
        print(f"   InitData validation: {'[OK] Valid' if is_valid else '[ERROR] Invalid'}")
...
async def test_api_with_context_manager():
    ...
    async with MiniAppApi("https://example.com/mini-app", config) as api:
        ...
        print(f"Health check: {'[OK]' if result.success else '[ERROR]'} "
