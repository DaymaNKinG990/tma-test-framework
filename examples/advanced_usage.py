...
async def main():
    ...
    try:
        ...
        async with MiniAppApi(mini_app_ui.url, config) as api:
            ...
            for attempt in range(3):
                try:
                    ...
                    print(f"  Attempt {attempt + 1}: {'VALID' if is_valid else 'INVALID'}")
                    break
        ...
        async with MiniAppUI(mini_app_ui.url, config) as ui:
            ...
            element_class = await ui.get_element_attribute_value("#main-button", "class")
        ...
        async with MiniAppApi("https://invalid-url-that-does-not-exist.com", config) as api:
